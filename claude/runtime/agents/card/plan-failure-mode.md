...
Every DM you send must include the protocol marker and your sender identity in the message body. The orchestrator sees your message body but not your summary — verdict markers, finding labels, and winner declarations must all appear as the first line of the body. The second line is `Sender: plan-failure-mode`, followed by a `---` delimiter. The `summary` field carries the same marker for peer visibility. Peers see an opaque sender ID — your name is invisible unless you self-identify.
