...
// ../sdk/src/bin/spawn-transcript-watcher.ts
import { spawn, spawnSync } from "node:child_process";
function spawnTranscriptWatcher(pid, sessionId, transcriptPath, cardId, cardRepoPath, logger2) {
  const readiness = spawnSync("sh", ["-c", "command -v transcript-watcher"], { stdio: "ignore" });
  if (readiness.error || readiness.status !== 0) {
    logger2.error("transcript-watcher not resolvable on PATH \u2014 skipping spawn", {
  ...
  child.on("error", (err) => {
    logger2.error("transcript-watcher spawn failed", {
...
function getCardRepoPidsLockPath() {
  return join2(getCardsDir(), "card-repo-commits", "pids.lock");
}
...
// src/session-start-after-compaction.ts
var ROUTING_REMINDER = "**IMPORTANT: Immediately load skills based on the `<routing-instructions>`.**";
var session_start_after_compaction_default = sessionStartHook({ matcher: "compact" }, async (input, { logger: logger2, persistEnvVar: persistEnvVar2 }) => {
  logger2.info("Post-compaction session start: checking for attach-mode watcher rotation");
  const agentPid = findAgentPid();
  if (agentPid == null) {
    logger2.info("Post-compaction session start: no agent PID found, injecting routing reminder only");
  ...
  const association = await getPidCardAssociation(agentPid).catch((err) => {
    logger2.warn("Post-compaction session start: failed to look up PID association", {
      pid: agentPid,
      error: err instanceof Error ? err.message : String(err)
    });
    return null;
  ...
  if (association == null || association.mode !== "attach") {
    logger2.info("Post-compaction session start: no attach-mode association, injecting routing reminder only", {
      pid: agentPid,
  ...
  logger2.info("Post-compaction session start: attach-mode \u2014 rotating transcript watcher", {
    pid: agentPid,
    cardId,
  ...
  try {
    ...
    if (apiInfo) {
      ...
      if (response.ok) {
        ...
        logger2.info("Post-compaction session start: stopped outgoing watcher", {
          cardId,
          stopped: body.stopped,
          timedOut: body.timedOut
        });
      } else {
        logger2.warn("Post-compaction session start: DELETE watcher returned non-OK", {
          cardId,
          status: response.status
        });
      }
    } else {
      logger2.warn("Post-compaction session start: extension API not available, skipping watcher stop", { cardId });
    }
  } catch (err) {
    logger2.warn("Post-compaction session start: failed to stop outgoing watcher (proceeding)", {
      cardId,
      error: err instanceof Error ? err.message : String(err)
    });
  }
  try {
    ...
    logger2.info("Post-compaction session start: registered new session", {
      pid: agentPid,
      sessionId: input.session_id
    });
  } catch (err) {
    logger2.warn("Post-compaction session start: failed to register new session (proceeding)", {
      pid: agentPid,
      sessionId: input.session_id,
      error: err instanceof Error ? err.message : String(err)
    });
  }
  persistEnvVar2("CARDS_SESSION_ID", input.session_id);
  persistEnvVar2("CARDS_TRANSCRIPT_PATH", input.transcript_path);
  logger2.info("Post-compaction session start: persisted updated session env vars", {
    sessionId: input.session_id,
  ...
  try {
    ...
    if (client) {
      ...
      spawnTranscriptWatcher(agentPid, input.session_id, input.transcript_path, cardId, cardRepoPath, logger2);
      logger2.info("Post-compaction session start: spawned new transcript watcher", {
        pid: agentPid,
        sessionId: input.session_id,
        cardRepoPath
      });
    } else {
      logger2.warn("Post-compaction session start: client unavailable, skipping watcher spawn", { cardId });
    }
  } catch (err) {
    logger2.warn("Post-compaction session start: failed to spawn new transcript watcher (proceeding)", {
      cardId,
      sessionId: input.session_id,
      error: err instanceof Error ? err.message : String(err)
    });
