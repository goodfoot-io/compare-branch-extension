...
**Re-validate.** During the fix-then-recheck loop, scope test runs to the failing test or suite per CLAUDE.md `<validation>`. The final certification before reporting COMPLETED runs the user's full validation command end-to-end; do not report COMPLETED until it exits 0.
