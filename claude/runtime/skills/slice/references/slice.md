...
Dispatch one sequential general-purpose subagent per slice. Sequential — never parallel. The diff-review and validation gate between slices is load-bearing.

```xml
<invoke name="Agent">
<parameter name="description">Slice [SLICE_LABEL]</parameter>
...
- Scope — did the subagent touch only files named in its brief?
...
- **Clean (review passes, validation passes)**: The slice is good. Commit the subagent's work if it left anything unstaged, mark the slice's task `completed`, and return to Step 4 with the next slice.
- **Trivially wrong within the same slice** (typo, missing import, off-by-one in the commit message) and the fix is obviously inside the boundary the subagent already touched: fix inline, re-validate, commit, advance.
- **Non-trivial wrong** (missed scope, wrong types, swallowed errors, ad-hoc skipped-test bodies, scope creep): do **not** fix inline. Dispatch a corrective general-purpose subagent with a focused prompt that names the specific defects and the files to change. Return to Step 5 on its return.

Do not advance the slice task to `completed` until validation passes cleanly.

### 5.4 Drift Check

After each successful slice, re-read the remaining slice sequence. If the last slice revealed consumers, dependencies, or constraints that invalidate the remaining sequence, revise the sequence before dispatching the next slice. If the revision requires a different strategy — not just reordering — read `<when-to-return-to-planning>` below.

## 6. Complete or Await Review

When every slice in the sequence is `completed`:
