...
- **Contest** — two or more approaches with real trade-offs, or unknowns deep enough that diverse exploration pays for itself. Read `./contest.md` and follow its instructions, choosing `[N_PLANNERS]` (2–4) by the size of the solution space and `[PLANNER_MODEL]` per slot. Un-approved plan files already in `plans/` favor a contest with incumbent seeding — the prior work gets graded rather than re-derived. On return:
