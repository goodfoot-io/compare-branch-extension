...
Load the `runtime:admiral` skill to re-evaluate the card's state and route to the appropriate next reference. On this second pass, skip the NOT DOR_MET route — enrichment has already been attempted this session.

```xml
<invoke name="Skill">
<parameter name="skill">runtime:admiral</parameter>
</invoke>
```
