...
Save the questions as a note per the `<take-notes>` instructions — slug `failure-mode-questions` — and commit before evaluating. A replacement evaluator reads this note instead of rediscovering the lens; on Deep depth, read your peer's questions note to deconflict lanes. The note is a floor, never a ceiling — every §3 sweep goes beyond it.
...
Marker: `FINDING: [short label] round-K` per `<dm-envelope>`. Round-K is the current evaluation round (round-1 on initial dispatch, round-2 after the first re-evaluation) — a private label so you can tell which round you first raised a finding in across resumes. The body carries cause / mode / effect, the severity / occurrence / detection tags, the file or runtime path, and the witness.
