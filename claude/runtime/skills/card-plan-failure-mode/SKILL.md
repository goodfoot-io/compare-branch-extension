...
Competing planners DM you directly with critiques of each other's plans using a `CRITIQUE: [label] for:planner-N` marker in `summary`. Treat each critique as a candidate finding, not a verified one:
...
The marker `FINDING: [short label] for:[PLANNER] round-K` goes in `summary` (round-tagged from the planner's most recent `PLAN: READY` round, so the planner can match each finding to a specific round under sticky-but-revocable approval). The body in `message` carries the finding with all three Step 3 components, plus the plan section or file it applies to.
...
<invoke name="SendMessage">
  <parameter name="to">[PLANNER]</parameter>
  <parameter name="summary">VERDICT: BLOCKED for:[PLANNER] because:<short cause></parameter>
  <parameter name="message">[Same body.]</parameter>
</invoke>
```
