...
`panel matches disk` compares two surfaces that share one source — the panel copies the disk stamp at HTML-generation time — so it cannot see the case where both are stale together. Only the host settles that, which is why the host read above is not optional:

```bash
cards-extension execute-command cards.debug.getBuildInfo
```

Compare its `buildTime` against the disk's. **Host newer than disk** means a `yarn watch` session is withholding its stamp: some build target has not produced a successful bundle this session, so `build-target.json` — and every panel copying it — still describes the last complete build while `dist/bundle.cjs` has been rebuilt from every save since. Do not reload and do not reinstall; that watch terminal prints `Build incomplete — no successful bundle this session for: <targets>` on every rebuild, and fixing those targets restamps on the next one. **Host older than disk** is the ordinary stale-host case: reload the window.
