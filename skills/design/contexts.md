...
A `.html` file in a card repository, rendered inside a sandboxed `srcdoc` iframe.

**Assumed environment (fact):** the iframe receives the live `--vscode-*` custom-property set plus `--cards-status-todo/-active/-needs-review/-done/-archived`, all forwarded and updated live on theme change. `data-vscode-theme-kind` (`light|dark|high-contrast`) is set on `<html>`.
...
- External `https://` assets (scripts, stylesheets, fonts, images) are allowed — any `https://` origin, no allowlist. `http://`, protocol-relative `//`, and relative paths are rejected at commit time and blocked by the CSP. Pin exact versions on CDN references; the stack above is the default baseline, not an exclusive allowlist.
- `connect-src https:` — `fetch`/XHR to any `https://` origin works; plain `ws://` and non-https endpoints are blocked.
- Inline scripts are auto-nonced by the panel — don't add your own `nonce` attribute. External `<script src="https://…">` runs on the `https:` scheme token, no nonce involved.
- `<base target="_blank">` is injected automatically — every link opens outside the iframe already.
- Sandbox has `allow-scripts` but **no** `allow-same-origin` — don't write code that assumes same-origin access (cookies, parent-frame DOM, storage).
