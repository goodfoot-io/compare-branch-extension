...
- Inline scripts are auto-nonced by the server at serve time, a fresh nonce per request — don't add your own `nonce` attribute. External `<script src="https://…">` runs on the `https:` scheme token, no nonce involved.
