...
async function checkBranchExists(repoRoot, branchName) {
  const { stdout } = await execFileAsync("git", ["branch", "--list", branchName], { cwd: repoRoot, timeout: 3e4 });
...
async function copyExistingSymlinks(sourceRoot, worktreeDir) {
  ...
  const symlinks = entries.filter(
    (e) => e.isSymbolicLink() && e.name !== ".git" && e.name !== ".worktrees"
  );
...
async function rerouteNodeModules(opts) {
  ...
  const counts = await Promise.all(entries.map(async (entry) => {
    ...
    if (entry.isSymbolicLink()) {
      ...
      const scopeCounts = await Promise.all(scopeEntries.map(async (scopeEntry) => {
...
async function updateGitExclude(opts) {
  ...
  const { stdout: gitDir } = await execFileAsync("git", ["-C", worktreeDir, "rev-parse", "--git-dir"], { timeout: 5e3 });
  ...
  try {
    await execFileAsync("git", ["-C", worktreeDir, "config", "--worktree", "core.excludesFile", excludePath], { timeout: 5e3 });
  } catch {
  }
}
