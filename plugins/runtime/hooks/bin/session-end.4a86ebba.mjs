...
// src/session-end.ts
var TranscriptUploadError = class extends Error {
...
var session_end_default = sessionEndHook({}, async (input, { logger: logger2 }) => {
  ...
  try {
    const transcript = await readFile2(input.transcript_path, "utf-8");
    const client = await createCardsClient(logger2);
    if (!client) {
      logger2.warn("API discovery failed, skipping transcript upload", { sessionId: input.session_id });
      return sessionEndOutput({});
    }
    ...
    await stream.close();
    logger2.info("Transcript uploaded", { sessionId: input.session_id, cardId: actionInput.cardId });
  } catch (error) {
    if (error && typeof error === "object" && "code" in error && error.code === "ENOENT") {
      logger2.warn("Transcript file not found", { path: input.transcript_path });
      return sessionEndOutput({});
    }
    const uploadError = new TranscriptUploadError(input.session_id, error);
    logger2.error("Transcript upload failed", { sessionId: uploadError.sessionId, error: uploadError.message });
  }
  return sessionEndOutput({});
});
