...
Write `CARD.md.meta.json` with `title` set to `"Description"` and `summary` (100–300 words: what is broken and the observable impact on users or the system). Follow the `<markdown-guidelines>` in the `cards:markdown` skill.
