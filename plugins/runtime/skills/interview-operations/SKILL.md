...
Write `CARD.md.meta.json` with `title` set to `"Description"` and `summary` (100–300 words: what is failing or at risk, the urgency, and the operational impact). Follow the `<markdown-guidelines>` in the `cards:markdown` skill for both `CARD.md` and the `summary` field.
