...
Adaptive Cards are interactive UI components for **structured decisions that need a recorded response**:

- Requesting plan approval (approve/reject with feedback)
- Presenting implementation options the user must choose between
...
Captured when users respond to an Adaptive Card.
...
Notes are **structured records** intended to outlive the current session:

- Architecture Decision Records (ADRs)
- Investigation findings that future agents should reference
- Session summaries capturing key decisions and rationale

Unlike comments (chronological), notes are **named and titled** for direct retrieval. Unlike Adaptive Cards, notes have no response mechanism.
...
sidecar auto-generated by the pre-commit hook. Do not create or edit these files manually.
