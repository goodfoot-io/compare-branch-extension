...
Write `CARD.md.meta.json` with `title` set to `"Description"` and `summary` (100–300 words: what is missing or outdated and who is affected by the gap). Follow the `<markdown-guidelines>` for both `CARD.md` and the `summary` field.
