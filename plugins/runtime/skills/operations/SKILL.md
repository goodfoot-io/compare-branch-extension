...
- **Evidence-grounded**: Symptoms, metrics, logs, dashboards, incident links; baselines (build time, failure rate, latency, cost). Link to configs, pipelines, and runbook files inline — soft links for prose (`the [deploy workflow](./infra/deploy.yml) triggers on merge`) or precise anchors for specific locations (`[infra/deploy.yml L45](./infra/deploy.yml#L45)`)
