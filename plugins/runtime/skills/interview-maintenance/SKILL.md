...
Write `CARD.md.meta.json` with `title` set to `"Description"` and `summary` (100–300 words: what technical debt or degradation exists and its measurable impact). Follow the `<markdown-guidelines>` in the `cards:markdown` skill for both `CARD.md` and the `summary` field.
