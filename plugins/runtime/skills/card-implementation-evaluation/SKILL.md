...
description: Evaluate implementation via failure-mode subagents. The orchestrator dispatches developer waves in response to findings and runs the validate-and-commit gate after each wave.
---
...
Group the findings by coherence, using the same routing principle as `runtime:card-implementation-with-plan` Step 2.2: Assess Coherence:
...
Choose [MODEL] per the same tiering as `runtime:card-implementation-with-plan` Step 2.3: Delegate Implementation.
