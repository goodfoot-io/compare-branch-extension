...
Write `CARD.md.meta.json` with `title` set to `"Description"` and `summary` (100–300 words: what needs to be learned, why the answer is not yet known, and what decision it unblocks). Follow the `<markdown-guidelines>` in the `cards:markdown` skill for both `CARD.md` and the `summary` field.
