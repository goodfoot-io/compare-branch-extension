...
async function notifySessionStart(sessionId, dispatcherPid, baseUrl) {
  const url = `${baseUrl}/session/start`;
  const method = "POST";
  try {
    ...
    if (!response.ok) {
      ...
      throw new ApiError("Notify session start failed", {
        url,
        method,
    ...
    throw new ApiError(`Notify session start error: ${error instanceof Error ? error.message : String(error)}`, {
      url,
      method,
      cause: error
    });
...
function isDispatcherAlive(pid) {
  try {
    process.kill(pid, 0);
    return true;
  } catch {
...
function signalActive(pid, logger2) {
  if (process.env.SIGNAL_ACTIVE_TEST_MODE === "1") {
    logger2?.debug("TEST_MODE: Would send SIGURG", { pid });
    return true;
  }
  try {
    ...
    logger2?.warn("Failed to signal dispatcher active", {
      pid,
      error: String(error)
    });
    return false;
...
// src/session-start-dispatcher-active.ts
var session_start_dispatcher_active_default = sessionStartHook({}, async (input, { logger: logger2 }) => {
