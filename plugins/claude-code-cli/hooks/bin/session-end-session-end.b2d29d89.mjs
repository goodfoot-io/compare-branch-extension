...
async function deleteSessionWatermark(sessionId, baseUrl, logger2) {
  try {
    ...
    logger2?.debug("Delete session watermark failed", { error: String(error) });
    return false;
  }
}
...
var session_end_session_end_default = sessionEndHook({}, async (input, { logger: logger2 }) => {
  ...
  const success = await deleteSessionWatermark(sessionId, baseUrl, logger2);
  if (!success) {
    logger2.warn("Failed to notify extension of session end");
  }
  return sessionEndOutput({
