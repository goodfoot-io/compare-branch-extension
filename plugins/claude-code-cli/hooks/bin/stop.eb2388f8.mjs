...
async function fetchIssueDiff(sessionId, issueId, baseUrl, logger2) {
  try {
    const response = await fetch(`${baseUrl}/session/${sessionId}/diff?issueIds=${issueId}`, {
...
function hasUpdates(diff) {
  return diff.issues.some((issue) => (issue.newComments?.length ?? 0) > 0 || (issue.fieldChanges?.length ?? 0) > 0);
}
function buildUpdateSummary(diff) {
  let commentCount = 0;
  let changeCount = 0;
  for (const issue of diff.issues) {
    commentCount += issue.newComments?.length ?? 0;
    changeCount += issue.fieldChanges?.length ?? 0;
  }
  const issuesWithUpdates = diff.issues.filter(
    (issue) => (issue.newComments?.length ?? 0) > 0 || (issue.fieldChanges?.length ?? 0) > 0
  );
  const issueCount = issuesWithUpdates.length;
  const parts = [];
  if (commentCount > 0) {
  ...
  if (issueCount === 1) {
    const issueTitle = issuesWithUpdates[0].issueTitle ?? "unknown";
...
var stop_default = stopHook({}, async (input, { logger: logger2 }) => {
  ...
  if (!baseUrl) {
    return stopOutput({
      decision: "approve",
      systemMessage: "Stop approved (API unavailable)"
    });
  }
  const diffResponse = await fetchIssueDiff(sessionId, issueId, baseUrl, logger2);
  if (!diffResponse) {
    return stopOutput({
      decision: "approve",
      systemMessage: "Stop approved (diff unavailable)"
    });
  }
  if (!hasUpdates(diffResponse)) {
    ...
    return stopOutput({
      decision: "approve",
      systemMessage: "Stop approved: No pending issue updates"
    });
  }
  const systemMsg = buildUpdateSummary(diffResponse);
  const diffJson = JSON.stringify(diffResponse);
  return stopOutput({
    decision: "block",
