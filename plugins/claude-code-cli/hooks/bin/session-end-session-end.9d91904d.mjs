...
async function deleteSessionWatermark(sessionId, baseUrl) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new ApiError("Delete session watermark failed", {
        url,
        method,
    ...
    throw new ApiError(`Delete session watermark error: ${error instanceof Error ? error.message : String(error)}`, {
      url,
      method,
      cause: error
    });
...
// src/session-end-session-end.ts
var session_end_session_end_default = sessionEndHook({}, async (input, { logger: logger2 }) => {
