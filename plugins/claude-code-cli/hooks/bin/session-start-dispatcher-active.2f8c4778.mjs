...
async function notifySessionStart(sessionId, dispatcherPid, baseUrl, logger2) {
  try {
    ...
    logger2?.debug("Notify session start failed", { error: String(error) });
    return false;
  }
}
...
var session_start_dispatcher_active_default = sessionStartHook({}, async (input, { logger: logger2 }) => {
  ...
  try {
    ...
    const success = await notifySessionStart(sessionId, dispatcherPid, baseUrl, logger2);
    if (!success) {
      logger2.warn("Failed to notify extension of session activity");
    }
  } catch (error) {
