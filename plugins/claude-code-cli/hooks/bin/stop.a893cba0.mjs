...
var ApiError = class _ApiError extends Error {
  constructor(message, options) {
    super(_ApiError.formatMessage(message, options ?? {}), options?.cause ? { cause: options.cause } : void 0);
    this.options = options;
  }
  name = "ApiError";
  static formatMessage(message, details) {
    const lines = [message];
    if (details.method && details.url) {
      lines.push(`  ${details.method} ${details.url}`);
    } else if (details.url) {
      lines.push(`  URL: ${details.url}`);
    }
    if (details.status !== void 0) {
      lines.push(`  Status: ${details.status}${details.statusText ? ` ${details.statusText}` : ""}`);
    }
    if (details.responsePreview) {
      lines.push(`  Response: ${details.responsePreview}`);
    }
    return lines.join("\n");
  }
};
function discoverApiUrl(logger2) {
  ...
  if (!pluginRoot) {
    ...
    throw new ApiError("API discovery failed: CLAUDE_PLUGIN_ROOT environment variable is not set");
  }
  const command = `"${pluginRoot}/bin/discover-workspace-api.sh"`;
  try {
    ...
    const stderr = execError.stderr ? typeof execError.stderr === "string" ? execError.stderr : execError.stderr.toString() : "";
    throw new ApiError(
      `API discovery script failed: ${command}${execError.status !== void 0 ? ` (exit code: ${execError.status})` : ""}${stderr ? `
  stderr: ${stderr.trim()}` : ""}`,
      { cause: error }
    );
  }
}
async function fetchIssueDiff(sessionId, issueId, baseUrl) {
...
async function notifySessionStop(sessionId, dispatcherPid, baseUrl) {
  const url = `${baseUrl}/session/stop`;
  const method = "POST";
  try {
    const response = await fetch(url, {
      method,
    ...
    if (!response.ok) {
      const responseText = await response.text();
      throw new ApiError("Notify session stop failed", {
        url,
        method,
        status: response.status,
        statusText: response.statusText,
    ...
    throw new ApiError(`Notify session stop error: ${error instanceof Error ? error.message : String(error)}`, {
      url,
      method,
      cause: error
    });
...
var stop_default = stopHook({}, async (input, { logger: logger2 }) => {
  ...
  const diffResponse = await fetchIssueDiff(sessionId, issueId, baseUrl);
  if (!hasUpdates(diffResponse)) {
    const dispatcherPid = getDispatcherPid();
    if (dispatcherPid) {
      await notifySessionStop(sessionId, dispatcherPid, baseUrl);
