...
function formatSkillList(skills) {
  const prefixed = skills.map((s) => `'claude-code-cli:${s}'`);
  if (prefixed.length === 1) {
    return prefixed[0];
  }
  if (prefixed.length === 2) {
    return `${prefixed[0]} and ${prefixed[1]}`;
  }
  const allButLast = prefixed.slice(0, -1);
  const last = prefixed[prefixed.length - 1];
...
function readCliSkills(sessionId, logger2) {
  const state = readState(sessionId, logger2);
  return state.cliSkills ?? [];
}

// src/session-start-state-reload.ts
var session_start_state_reload_default = sessionStartHook({ matcher: "compact" }, (input, { logger: logger2 }) => {
  const sessionId = input.session_id;
  if (!sessionId) {
    return sessionStartOutput({});
  }
  const cliSkills = readCliSkills(sessionId, logger2);
  if (cliSkills.length === 0) {
    return sessionStartOutput({});
  }
  const skillList = formatSkillList(cliSkills);
  let systemMsg;
  if (cliSkills.length === 1) {
    systemMsg = `Reloading skill: claude-code-cli:${cliSkills[0]}`;
  } else {
    const skillNames = cliSkills.map((s) => `claude-code-cli:${s}`).join(", ");
    systemMsg = `Reloading skills: ${skillNames}`;
  }
  const context = `Load skill ${skillList} before continuing`;
  return sessionStartOutput({
