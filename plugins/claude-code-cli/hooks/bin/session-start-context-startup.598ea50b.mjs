...
async function fetchIssue(issueId, baseUrl) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new ApiError("Issue fetch failed", {
        url,
        method,
    ...
    throw new ApiError(`Issue fetch error: ${error instanceof Error ? error.message : String(error)}`, {
      url,
      method,
      cause: error
    });
...
var session_start_context_startup_default = sessionStartHook({ matcher: "startup" }, async (input, { logger: logger2 }) => {
  ...
  const issue = await fetchIssue(issueId, baseUrl);
