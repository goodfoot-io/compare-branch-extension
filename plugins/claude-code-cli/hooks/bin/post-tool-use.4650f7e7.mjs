...
function addCliSkill(sessionId, skillName, logger2) {
  const state = readState(sessionId, logger2);
  const skills = new Set(state.cliSkills ?? []);
  skills.add(skillName);
  state.cliSkills = [...skills];
  writeState(sessionId, state, logger2);
}

// src/post-tool-use.ts
var post_tool_use_default = postToolUseHook({ matcher: "Skill" }, (input, { logger: logger2 }) => {
