...
async function postSessionComment(issueId, sessionId, baseUrl) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new ApiError("Post session comment failed", {
        url,
        method,
    ...
    throw new ApiError(`Post session comment error: ${error instanceof Error ? error.message : String(error)}`, {
      url,
      method,
      cause: error
    });
