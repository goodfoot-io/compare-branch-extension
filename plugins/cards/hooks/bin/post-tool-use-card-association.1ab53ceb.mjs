...
var REGISTRY_PATH = getRegistryPath();
var LOCK_PATH = getLockPath();
...
function findClaudePid(startPid) {
  ...
  for (let depth = 0; depth < PROCESS_TREE_MAX_DEPTH; depth++) {
    ...
    try {
      ...
      if (basename(comm).toLowerCase() === "claude") {
        return pid;
      }
      const args = execSync(`ps -p ${pid} -o args=`, { encoding: "utf8" }).trim();
      if (/\bclaude\b/i.test(args)) {
        return pid;
      }
    } catch {
      return null;
    ...
    try {
      ...
      if (Number.isNaN(parentPid) || parentPid === pid) {
        return null;
      }
      pid = parentPid;
    } catch {
...
var post_tool_use_card_association_default = postToolUseHook({ matcher: "Bash" }, async (input, { logger: logger2 }) => {
  ...
  try {
    ...
    let isWriteMethod = false;
    const explicitMethodMatch = command.match(/-X\s+(\w+)|--request\s+(\w+)/);
    if (explicitMethodMatch) {
      const method = (explicitMethodMatch[1] ?? explicitMethodMatch[2])?.toUpperCase() ?? "";
      isWriteMethod = ["POST", "PUT", "PATCH", "DELETE"].includes(method);
    } else {
      const implicitPostMatch = /(?:^|\s)(?:-d|--data|--data-raw|--data-binary)(?:\s|=)/.test(command);
      isWriteMethod = implicitPostMatch;
    }
    if (!isWriteMethod) {
      return postToolUseOutput({});
    }
    const cardIdMatch = command.match(/https?:\/\/(?:localhost|127\.0\.0\.1)(?::\d+)?\/(?:.*\/)?cards\/([^/\s"']+)/);
    if (!cardIdMatch?.[1]) {
      return postToolUseOutput({});
    }
