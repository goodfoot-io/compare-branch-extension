...
function discoverApiInfo(logger2) {
  ...
  const configPath = join(homedir(), ".compare-branch", "issues-v2-api.json");
  try {
...
function discoverApiUrl(logger2) {
  ...
  const info = discoverApiInfo(logger2);
  if (info) {
...
async function checkRemoteUpdates(issueId, baseUrl, since, logger2) {
  if (process.env["API_TEST_MODE"] === "1") {
    logger2?.debug("API_TEST_MODE: Returning false for checkRemoteUpdates");
    return false;
  }
  try {
    const url = since ? `${baseUrl}/issues/${issueId}/has-updates?since=${encodeURIComponent(since)}` : `${baseUrl}/issues/${issueId}/has-updates`;
    const response = await fetch(url, { signal: AbortSignal.timeout(5e3) });
    if (!response.ok) return false;
    const data = await response.json();
    return data.hasUpdates;
  } catch (error) {
    logger2?.debug("Check remote updates failed", { error: String(error) });
    return false;
  }
}
