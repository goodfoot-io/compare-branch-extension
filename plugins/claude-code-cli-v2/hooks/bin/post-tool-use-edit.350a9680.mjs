...
// ../issues-validator/src/card.ts
var VALID_CARD_STATUSES = ["active", "completed", "cancelled"];
...
function validateCard(card) {
  ...
  } else if (!VALID_CARD_STATUSES.includes(card.status)) {
    errors.push({
      field: "status",
      message: `status must be one of: ${VALID_CARD_STATUSES.join(", ")}`,
      code: "invalid_status"
    });
  }
  ...
  } else if (typeof card.card !== "object" || Array.isArray(card.card)) {
    errors.push({
      field: "card",
...
// ../issues-validator/src/frontmatter.ts
var VALID_TASK_STATUSES = ["pending", "in_progress", "needs_review", "done", "blocked"];
var MAX_SUMMARY_LENGTH2 = 200;
function validateTaskContent(rawMarkdown) {
  const errors = [];
  ...
  } else if (data["summary"].length > MAX_SUMMARY_LENGTH2) {
    errors.push({
      field: "summary",
      message: `Field "summary" must not exceed ${MAX_SUMMARY_LENGTH2} characters`,
      code: "length_exceeded"
    });
  }
  if (!("status" in data)) {
  ...
  } else if (!VALID_TASK_STATUSES.includes(data["status"])) {
    errors.push({
      field: "status",
      message: `Field "status" must be one of: ${VALID_TASK_STATUSES.join(", ")}`,
      code: "invalid_status"
    });
  }
  if (!("dependencies" in data)) {
    errors.push({
      field: "dependencies",
      message: 'Field "dependencies" is required',
      code: "missing_field"
    });
...
var post_tool_use_edit_default = postToolUseHook({ matcher: "Write|Edit|MultiEdit" }, async (input, { logger: logger2 }) => {
  ...
  const isTaskFile = filePath.includes("/tasks/") && filePath.endsWith(".md");
  const isCardFile = filePath.includes("/cards/") && filePath.endsWith(".json");
  const isIssueFile = filePath.endsWith("/ISSUE.md");
  if (!isTaskFile && !isCardFile && !isIssueFile) {
    return postToolUseOutput({});
  }
  try {
    const content = await readFile(filePath, "utf-8");
    if (isTaskFile || isIssueFile) {
      const result = validateTaskContent(content);
      if (!result.valid) {
        return postToolUseOutput({
          systemMessage: `Validation failed: ${result.errors.map((e) => e.message).join(", ")}`
        });
      }
    }
    if (isCardFile) {
...
// ../../../../../tmp/claude-code-hooks-build/e5d499fbc22951d8/wrapper.ts
