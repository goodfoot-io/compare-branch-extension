...
function buildArgs(prompt, sessionId, resume, mode, cardRepoPath, extensionPath) {
  const args = [];
  if (resume) {
  ...
  args.push("--plugin-dir", path2.join(extensionPath, "dist", "plugins", "runtime"));
...
var launch_default = defineAction(
  ...
  async (input, context) => {
    ...
    const args = buildArgs(prompt, sessionId, resume, input.executionMode, input.cardRepoPath, input.extensionPath);
