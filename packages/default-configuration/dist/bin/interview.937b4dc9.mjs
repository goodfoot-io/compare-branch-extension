...
async function copyCardsDirectory(sourceRoot, worktreeDir) {
  const sourcePath = path2.join(sourceRoot, ".cards");
  try {
    await fs2.cp(sourcePath, path2.join(worktreeDir, ".cards"), { recursive: true });
  } catch (error) {
    if (error.code !== "ENOENT") {
      throw error;
    }
