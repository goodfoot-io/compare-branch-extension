...
/**
 * Tests for centralized API discovery in the SDK client module.
 *
 * @summary Tests api-discovery in client
 */
...
import type { DiscoveryLogger } from '../../src/client/api-discovery.js';
import { createCardsClient, discoverApiInfo } from '../../src/client/api-discovery.js';

const mockLogger: DiscoveryLogger = {
  debug: vi.fn()
};
...
describe('api-discovery', () => {
  ...
  describe('discoverApiInfo', () => {
    ...
    it('should return null when accessToken is not a string', async () => {
      mkdirSync(join(testDir, '.cards'), { recursive: true });
      writeFileSync(
        join(testDir, '.cards', 'cards-api.json'),
        JSON.stringify({
    ...
    it('should return null when pid is not a number', async () => {
      mkdirSync(join(testDir, '.cards'), { recursive: true });
      writeFileSync(
        join(testDir, '.cards', 'cards-api.json'),
        JSON.stringify({
    ...
    it('should return null when startedAt is not a string', async () => {
      mkdirSync(join(testDir, '.cards'), { recursive: true });
      writeFileSync(
        join(testDir, '.cards', 'cards-api.json'),
        JSON.stringify({
