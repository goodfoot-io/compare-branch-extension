/**
 * Reproduction for: bindCard() passes agentPid: process.pid to
 ...
 * seconds after bind, the adhoc-cleanup process sees the PID die and
 * flips the card to needs_review prematurely.
 *
 * Hypothesis under test: bindCard() hard-codes agentPid: process.pid
 ...
 *
 * @summary bindCard passes CLI process.pid as agentPid instead of letting outfitWorktreeForCard resolve it
 */
...
describe('bindCard agentPid (CLI PID vs agent PID)', () => {
  let testDir: string;
  let server: Server;
  ...
  it('does not pass agentPid to outfitWorktreeForCard, letting it resolve the real agent PID via findAgentPid', async () => {
    ...
    try {
      ...
      // is the OutfitWorktreeForCardOptions object. Current buggy code at bindCard L1253
