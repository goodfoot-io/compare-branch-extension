...
export default subagentStartHook({}, async (input) => {
  ...
  const nudge = `If you know the id of an existing card this work belongs to, run \`cards <id> attach\` to attach it. Otherwise, load the \`cards:cards\` skill to create a new card for these changes.`;
