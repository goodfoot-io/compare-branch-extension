...
export default postToolUseHook({ matcher: 'EnterWorktree' }, async (input, { logger }) => {
  ...
  const systemMessage = `If you know the id of an existing card this work belongs to, run \`cards <id> attach\` to attach it. Otherwise, load the \`cards:cards\` skill to create a new card for these changes.`;
  const additionalContext = `If you know the id of an existing card this work belongs to, run \`cards <id> attach\` to attach it. Otherwise, load the \`cards:cards\` skill to create a new card for these changes.`;
