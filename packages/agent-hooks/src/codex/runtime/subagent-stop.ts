 ...
 *
 * Fails open — upload failure is non-fatal since transcript data may be
 * partially available via other means.
 *
...
export default subagentStopHook({}, async (input, { logger }) => {
  ...
  if (input.agent_transcript_path === null) {
    logger.warn('No agent transcript path provided; skipping upload', {
      sessionId: input.session_id,
      agentId: input.agent_id
    });
    return undefined;
