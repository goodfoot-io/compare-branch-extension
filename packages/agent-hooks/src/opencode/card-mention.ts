 ...
 *
 * Pure predicates ported verbatim from the Claude/Codex
 * `user-prompt-submit` handlers, where they live inline because each agent's
 * hook framework wraps them separately. The OpenCode bundle needs them over
 * `chat.message` payloads instead of hook stdin, so they are factored into one
 * testable module here. Behavior is byte-for-byte the siblings': whitespace-
 * bounded card terms, verb-anchored creation intent within a five-token
 * proximity window, and last-hyphen card-ID validation confirmed against
 * `~/.cards/cards-repos/`.
 *
 ...
 * `@cards.management/sessions/card-repo` don't match.
 *
 * @param prompt - The user's prompt text to scan.
 ...
 * lowercases it without touching internal characters.
 *
 ...
 * "cards" term, in either order. Verb-anchored only.
 *
 * @param prompt - The user's prompt text to scan.
 ...
 * `<cards-extension>` — identical wording to the sibling agents' hooks.
 *
