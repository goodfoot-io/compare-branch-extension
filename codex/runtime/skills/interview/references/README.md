...
- `<type>.md` — **writing guide**: target CARD.md structure (symlinks to `$management` references — edit there).

Plus one shared reference:
- `commanders-intent.md` — first-principles for the opening paragraph of CARD.md (symlink to `$management` references).
...
Writing guides and `commanders-intent.md` are symlinked from `$management` references — edit those files there; the symlinks ensure both skills see the same content.
