...
[PLAN_STATE=missing: explain that this card requires a plan (gates.planRequired=true) but none is written. Ask the user to run the Launch action so $card can route through card-plan, or to clear gates.planRequired if slicing without a plan is appropriate.]
