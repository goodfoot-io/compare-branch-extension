...
Write your plan to `[PLAN_FILE] = plan/[PLANNER_NAME].md` per the `$markdown` guidelines, with a sidecar `[PLAN_FILE].meta.json` whose `title` is `"Plan: <≤10 words>"`. Commit the plan file with a single sentence summarizing the approach.
