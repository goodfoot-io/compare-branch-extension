...
Load the `$markdown` skill. Write a comment to the card repository with the answer. Fragment-link every named file, function, and type per `<markdown-guidelines>`.
...
[answer to the question with fragment links for code references per the $markdown skill]
EOF
