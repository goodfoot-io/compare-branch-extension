...
**On any failure:** Create todos with "[Pre-eval fix]" prefix from all validation failures. **Delegate them — do not implement directly.** Return to Step 2.2 of `./implementation-with-plan.md`, then assess and delegate the new todos to a developer agent via Steps 2.3–2.4. After fixes, return to Step 1.
...
For each finding:
- **Viable**: Create a todo with "[Review fix]" prefix. **Delegate — do not implement directly.** Return to Step 2.2 of `./implementation-with-plan.md`, then assess and delegate via Steps 2.3–2.4.
