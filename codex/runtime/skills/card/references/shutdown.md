...
```bash
CARDS_SESSION_ID='<SESSION ID FROM THE STOP MESSAGE>' node "$(dirname "$(realpath "<ABSOLUTE shutdown.md PATH FROM THE STOP MESSAGE>")")/../bin/shutdown-codex.mjs"
```

Replace both angle-bracketed placeholders with the session id and absolute `shutdown.md` path from the Stop-hook message. Do not search for or guess a PID yourself.

The executable must inherit `CARD_REPO_PATH`. It accepts an inherited `CODEX_SESSION_ID` or `CARDS_SESSION_ID`; the command explicitly supplies `CARDS_SESSION_ID` from the trusted Stop event so the audit identity never depends on ambient availability. It selects and revalidates the exact Node launcher ancestor for the installed `@openai/codex/bin/codex.js`, sends that positive PID exactly `SIGTERM`, waits for its observed exit, and appends an audit record to `CARD_REPO_PATH/streams/codex-shutdown/<session-id>.jsonl`.
